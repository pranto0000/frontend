...
const AdminResultManagement = () => {
  ...
  const handleApproval = async (id, status) => {
    try {
      await axios.put(`https://school-4ee7.onrender.com/api/results/approve/${id}`, { status });
      fetchResults();
    } catch (error) {
                  ...
                  <tr key={result._id} className="border-b hover:bg-gray-50 transition-colors text-center">
                    ...
                    <td className="border px-4 py-2">
                      {result.status === "pending" && (
                        <>
                          <button onClick={() => handleApproval(result._id, "approved")} className="bg-green-500 text-white px-2 py-1 rounded mr-2">
                            Approve
                          </button>
                          <button onClick={() => handleApproval(result._id, "rejected")} className="bg-red-500 text-white px-2 py-1 rounded">
                            Reject
                          </button>
                        </>
                      )}
